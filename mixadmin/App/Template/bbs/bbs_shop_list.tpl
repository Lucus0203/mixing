...
<td valign="top" align="center">
 	<div class="main_ta_box">
         <div class="hd_t">{$shop.title}留言</div>
             ...
             <tr>
                 ...
                 <td style="word-break:keep-all;">
                 	{if $list[sec].allow neq 1}
                 		<a class="pubBtn" href="{url controller=Bbs action=Allow type=shop id=$list[sec].id}">通过</a>
                 	{else}
                 		<a class="depubBtn" href="{url controller=Bbs action=DeAllow type=shop id=$list[sec].id}">不通过</a>
                 	{/if}
                 		<a class="delBtn" href="{url controller=Bbs action=Del type=shop id=$list[sec].id}">删除</a>
                 </td>
             </tr>
             {/section}
         </table>
