...
function filecheck()
	{
		...
		if(file_exists($filename))
		{
			...
			{
				$this->errormsg.=$filename.'do not permit download ！';
				if($this->debug) exit($filename.'Do not permit download！') ;
				return false;
			}else
			{
				...
				{
					$this->errormsg.='Get '.$filename.' file type error';
		...
		{
			$this->errormsg.=$filename.' is not exit!';
			if($this->debug) exit($filename.' is not exit!') ;
			return false;
		}
	}
