...
<td valign="top" align="center">
 	<div class="main_ta_box">
         ...
         <form action="{url controller=Base action=AddQuestion}" method="post">
            ...
            <colgroup>
             ...
             <tr>
                 ...
                 <td style="word-break:keep-all;">
                 	<a href="{url controller=Base action=EditQuestion id=$list[sec].id}">编辑</a><a class="delBtn" href="{url controller=Base action=DelQuestion id=$list[sec].id}">删除</a>
                        {if $list[sec].recommend eq 1}<a href="{url controller=Base action=RecommendQuestion id=$list[sec].id}">推荐</a>{else}<a href="{url controller=Base action=UnRecommendQuestion id=$list[sec].id}">不推荐</a>{/if}
                 </td>
             </tr>
             {/section}
         </table>
