...
function shopsMap() {
    ...
    if(empty($lng) || empty($lat)){
        echo json_result(null, '2', '获取不到您的位置');
        return;
    }
    if(empty($zoomlevel)){
        echo json_result(null, '3', '获取不到您的放大级别');
        return;
    }
    ...
    $sql.=(!empty($keyword)) ? " and ( INSTR(title,'" . addslashes($keyword) . "') or INSTR(subtitle,'" . addslashes($keyword) . "') or INSTR(address,'" . addslashes($keyword) . "') ) " : '';
    $sql.=(!empty($tag_ids)) ? " and shop_tag.tag_id in ({$tag_ids}) " : '';
    $sql.=" and round(6378.138*2*asin(sqrt(pow(sin( ($lat*pi()/180-shop.lat*pi()/180)/2),2)+cos($lat*pi()/180)*cos(shop.lat*pi()/180)* pow(sin( ($lng*pi()/180-shop.lng*pi()/180)/2),2)))*1000) <= ".($zoom * 6);
    ...
    $sql .= " group by shop.id ";
    $sql .= " order by sqrt(power(lng-{$lng},2)+power(lat-{$lat},2)),id ";
    $shops = $db->getAllBySql($sql);
